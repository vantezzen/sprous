...
it("renders correctly", () => {
  ...
  const { container, getByTestId, getByText } = render(<Navigation />)
...
it("hides BTS link", () => {
  ...
  StaticQuery.mockImplementation(({ render }) => render(customAnswer))
  ...
  expect(container).toMatchSnapshot()
  expect(
    container.querySelector(`[data-testid="navigation-bts-link"]`)
...
it("uses BTS link config", () => {
  ...
  expect(getByTestId("navigation-bts-link")).toHaveAttribute(
    "href",
    "CUSTOM_HREF"
  )
})
...
it("uses uses image logo", () => {
  ...
  expect(container).toMatchSnapshot()
  expect(
    container.querySelector(`[data-testid="navigation-text-logo"]`)
  ).not.toBeInTheDocument()
